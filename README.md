...
It installs three packages and binds in well under a second. The server ships
compiled, so nothing is built at startup, and the only runtime dependencies are
the HTTP layer.
